...
import TableFilters from '../AssetManagementFilters';
...
export default function AssetManagementTableData({ isFlotaActivos = false }) {
  ...
  const filteredData = useMemo(() => {
    return allData.filter(item => {
      const matchesSearch         = !filters.search         || item.id.toString().includes(filters.search) || item.plate.includes(filters.search);
      ...
      const matchesClass          = !filters.classType      || item.class.includes(filters.classType);
      const matchesModel          = !filters.model          || item.model.toString().includes(filters.model);
      const matchesAdministration = !filters.administration || item.administration.includes(filters.administration);

      // algo ocurre con el status, pendiente revisar
      const matchesStatus         = !filters.status         || item.status.includes(filters.status);
  ...
  return (
    <>
      ...
      {/* Tabla */}
      <Table columns={columns} data={filteredData} isFlotaActivos={isFlotaActivos} />
    </>
  );
}
